...
void main()
{
    ...
    if (profit <= 100000) //低于或等于10万元时,奖金可提10%
        bonus = profit * 0.1;
    else if (profit <= 200000) //利润高于10万元，低于20万元时,低于10万元的部分按10%提成，高于10万元的部分，提成7.5%
        bonus = bonus1 + (profit - 100000) * 0.075;
    else if (profit <= 400000) //20万到40万之间时，高于20万元的部分，提成5%
        bonus = bonus2 + (profit - 200000) * 0.05;
    else if (profit <= 600000) //40万到60万之间时高于40万元的部分，可提成3%
        bonus = bonus4 + (profit - 400000) * 0.03;
    else if (profit <= 1000000) //60万到100万之间时，高于60万元的部分，可提成1.5%
